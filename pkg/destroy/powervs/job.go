...
func (o *ClusterUninstaller) listJobs() (cloudResources, error) {
	...
	jobs, err = o.jobClient.GetAll(o.ServiceGUID)
	if err != nil {
...
func (o *ClusterUninstaller) deleteJob(item cloudResource) error {
	...
	job, err = o.jobClient.Get(item.id, o.ServiceGUID)
	if err != nil {
	...
	err = o.jobClient.Delete(item.id, o.ServiceGUID)
	if err != nil {
...
func (o *ClusterUninstaller) destroyJobs() error {
	...
	if items = o.getPendingItems(jobTypeName); len(items) > 0 {
		return errors.Errorf("Error: destroyJobs: %d items pending", len(items))
	}
	return nil
}
