...
func (o *ClusterUninstaller) listImages() (cloudResources, error) {
	...
	images, err := o.imageClient.GetAll(o.ServiceGUID)
	if err != nil {
...
func (o *ClusterUninstaller) deleteImage(item cloudResource) error {
	...
	img, err = o.imageClient.Get(item.id, o.ServiceGUID)
	if err != nil {
	...
	err = o.imageClient.Delete(item.id, o.ServiceGUID)
	if err != nil {
...
func (o *ClusterUninstaller) destroyImages() error {
	...
	if items = o.getPendingItems(imageTypeName); len(items) > 0 {
		return errors.Errorf("Error: destroyImages: %d items pending", len(items))
	}
	return nil
}
